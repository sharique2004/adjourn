...
| `living-room-standup.jsonl` | **The demo meeting** (2026-08-21), ~3 minutes, 28 segments, two speakers. |
| `living-room-standup.fixtures.json` | Hand-written ground truth — 11 statements covering all ten statement kinds. |
...
```json
{"type":"meta","meeting_id":"living-room-standup","title":"Adjourn standup — living room","date":"2026-08-21"}
{"type":"segment","segment_id":"lr-s05","speaker":"Them","track":"system","start":14.3,"end":31.8,"text":"..."}
```
...
- **Every `quote` is verbatim** from the transcript segment. A quote is what the
  ...
  unauditable.
- **`entity_refs` only carries handles spoken *in that segment*.** `lr-s21` has
  empty refs even though it is about issue #1, because "issue one" is spoken in
  `lr-s20`. Inferring across segments is how a comment lands on the wrong issue.
...
| --- | --- | --- | --- |
| Redis dropped for an in-process LRU on issue #2 | `lr-s05` | `decision` | `github_update` (conflict vs. last week) |
| Priya takes the cache work off Sam | `lr-s09` | `assignment` | `github_update` (reassignment on #2) |
| Ship date moves to the twenty-fifth | `lr-s11` | `deadline` | `calendar_hold` |
| "We need a ticket for webhook signature verification" | `lr-s13` | `ticket_request` | `linear_create` |
| "SHA-5 is about eighty percent done" | `lr-s15` | `progress_report` | `linear_move` (In Progress -> In Review) |
| "I'll open a PR for the config loader tonight" | `lr-s17` | `pr_intent` | `pull_request_stub` |
| Auth migration running clean | `lr-s21` | `update` | `github_update` |
| Is SHA-5 the same as the rate-limit ticket? | `lr-s22` | `question` | — (recap only) |
| "I'll Slack the channel the summary" | `lr-s24` | `message_commitment` | `slack_send` (60s regret window) |
| "I'll email Alex the deck" | `lr-s25` | `email_commitment` | `email_send` (60s regret window) |
| "Let's review Friday" | `lr-s26` | `deadline` | `calendar_hold` |
...
`living-room-standup` reverses all three. Seed the prior meeting into memory before
...
* `lr-s15` says **SHA-5** verbatim, because SHA-5 ("Migrate transcript ingestion
...
* `lr-s13` asks for a ticket for **webhook signature verification**, NOT for
...
It is a copy of `living-room-standup.fixtures.json`. If the demo meeting changes,
